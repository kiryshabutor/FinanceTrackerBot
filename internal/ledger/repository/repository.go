...
func (r *Repository) GetCategoryByName(ctx context.Context, name, categoryType string, userID sql.NullInt64) (*Category, error) {
	var category Category

	query := `
		SELECT id, user_id, name, type, created_at
		FROM categories
		WHERE name = $1 AND type = $2 AND (user_id = $3 OR (user_id IS NULL AND $3 IS NULL))
		LIMIT 1
	`

	var err error
	if userID.Valid {
		err = r.db.QueryRow(ctx, query, name, categoryType, userID.Int64).Scan(
			&category.ID,
			&category.UserID,
			&category.Name,
			&category.Type,
		...
		err = r.db.QueryRow(ctx, query, name, categoryType, nil).Scan(
			&category.ID,
			&category.UserID,
			&category.Name,
			&category.Type,
